...
'use strict';
utilsModule
  .factory('HttpAppService', ['$log', '$http', 'setting', 'Prompter', function ($log, $http, setting, Prompter) {
    var serivieName = "HttpAppService";
    ...
    var request = {
      ...
      pluginLogin: function (userName, password, success, error) {
        ...
        CRCHttpClient.login(userName, password, successCallback, errorCallback);
      },
      pluginPost: function (apicode, token, arg1, arg2, arg3, success, error, state) {
        var successCallback = function (response) {
          //userToken失效,请重新登录
          if (response.returnCode == "ME000AM03") {
            goBackLogin(state);
            Prompter.showShortCenterToast("登录时间过长，请重新登录！");
          } else {
            success(response);
          }
        }
        ...
        CRCHttpClient.post(successCallback, errorCallback, apicode, token, setting.getUserToken(), arg1, arg2, arg3);
      },
      noAuthorPost: function (url, paramter) {
        $log.debug(noAuthorPostName + " Start!");
        $log.debug(noAuthorPostName + " url " + url);
        $log.debug(noAuthorPostName + " paramter " + angular.toJson(paramter));
        var noAuthorPost = $http.post(url, paramter).success(function (response) {
          $log.debug(noAuthorPostName + " success");
          $log.debug(noAuthorPostName + " response " + angular.toJson(response));
          $log.debug(noAuthorPostName + " End!");
        }).error(function (response, status) {
          $log.debug(noAuthorPostName + " error");
          $log.debug(noAuthorPostName + " response " + response);
          $log.debug(noAuthorPostName + " status " + status);
          $log.debug(noAuthorPostName + " End!");
        });
        return noAuthorPost;
      ...
      noAuthorGet: function (url) {
        $log.debug(noAuthorGetName + " Start!");
        $log.debug(noAuthorGetName + " url " + url);
        var get = $http.get(url).success(function (response) {
          $log.debug(noAuthorGetName + " success");
          $log.debug(noAuthorGetName + " response " + angular.toJson(response));
          $log.debug(noAuthorGetName + " End!");
        }).error(function (response, status) {
          $log.debug(noAuthorGetName + " error");
          $log.debug(noAuthorGetName + " response " + response);
          $log.debug(noAuthorGetName + " status " + status);
          $log.debug(noAuthorGetName + " End!");
        });
        return get;
      },
  ...
  .service('Prompter', ['$ionicLoading', '$cordovaToast', '$ionicPopup',
  ...
  .factory("test",['$compile',function($compile){
